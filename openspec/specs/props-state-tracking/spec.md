# Props State Tracking Specification

## Purpose
提供道具状态跟踪功能，记录道具获得、失去、转移、修改等变化，并在时间线上显示这些变化。
...
- **AND** shows change in properties if modified
...
The system SHALL generate paragraph state summary showing only items held by characters present in paragraph.
...
class PropsStateManager {
    // Get item changes for a paragraph
    ...
    // Get item state at specific paragraph (cumulative)
...
interface ItemState {
    action: 'acquire' | 'lose' | 'transfer' | 'modify' | 'none';
    properties: Record<string, string>;
    characterId?: string;
    location?: string;
}
